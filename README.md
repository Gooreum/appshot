...
**맥은 가로 캔버스이고 창 목업을 쓴다.** 2880×1800(16:10)이라 글자 크기는 캔버스 폭이 아니라
**짧은 변** 기준으로 잡고(폭 기준이면 헤드라인이 캔버스 높이의 9%가 된다), 레이아웃 치수는
`landscape` 값으로 갈아 끼운다. 목업은 노트북 베젤이 아니라 신호등 있는 **macOS 앱 창**이다 —
창 캡처는 비율이 제각각인데 창이 소스 비율을 그대로 따라가므로 잘리지 않는다.
...
¹ 6.9"와 6.5" 중 하나가 필수다. ² iPad에서 도는 앱일 때. ³ Mac 앱일 때 (2560×1600·1440×900·1280×800도 허용).
기기 이름은 App Store Connect 슬롯별 기기 목록을 따른다.
...
`theme.background`로 정한다. 그라디언트가 기본값이고, 준비한 이미지를 깔 수도 있다.
...
| `fit` | `"cover"`(기본) · `"contain"` | cover는 캔버스를 채우고 가장자리를 자른다. contain은 다 보이는 대신 여백(#111)이 남는다 |
| `overlay` | `0`~`1`, 기본 `0.28` | 배경 위 가독성 마스크. `0`이면 없음 |
| `noise` | `true`(기본) | 그라디언트 밴딩 제거용 2% 노이즈 |
| `panorama` | `false`(기본) | 배경이 여러 장에 걸쳐 이어진다. 그라디언트·이미지 모두 동작 |

**배경 이미지는 직접 준비해야 한다.** appshot은 이미지를 만들지 않고, 받은 파일을 규격에 맞게 깔고 검사한다.
배경 사진의 대비는 자동으로 검사할 수 없어서(부분마다 밝기가 다르다) 마스크를 기본으로 씌운다.
...
배경 이미지
- `bg-lowres` — 배경 폭이 캔버스보다 작아 확대되어 흐려짐
- `bg-crop` — cover인데 비율이 캔버스와 20% 넘게 달라 가장자리가 잘림
- `bg-unused` — 모든 장이 fullbleed라 배경 이미지가 보이지 않음
...
맥은 `--platform macos`로 **창을 클릭해** 찍는다(`screencapture -w -o`). 화면 전체를 찍지 않으므로
열려 있던 다른 창이 스토어에 올라갈 일이 없다. 창 캡처에는 메뉴바가 없어 정리할 상태바도 없다.
