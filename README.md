...
대신 CSS 프레임은 실물 사진만큼 정교하지 않다. 그래서 프레임 수치는 감으로 잡지 않고 실물 사진 기반
목업을 픽셀로 재서 옮겼다. iPhone은 금속 밴드 1.4%, 밴드와 유리 베젤을 합쳐 3.7%, 버튼 돌출 0.7%이고,
iPad는 은색 밴드 0.4%와 짙은 회색 베젤을 합쳐 4.3%다. 밴드는 대각선 그라디언트 한 장이 아니라
바깥 그늘 → 바탕 → 안쪽 하이라이트 → 홈 → 유리 순서의 링으로 칠한다. 둥근 금속 단면이 그렇게 보이기
때문이다. 처음에는 밴드를 그라디언트 한 장으로 칠하고 베젤을 1%대로 얇게 잡았는데, 결과가 플라스틱
테두리처럼 보였다. 최소한 "기기로 읽히는 수준"을 렌더마다 확인하는 것이 아래 프레임 게이트의 역할이다.
